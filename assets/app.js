...
(() => {
  ...
  function renderDesktop() {
    ...
    $('#desktop-apps').innerHTML=children.map(node=>{
      ...
      return `<button class="desktop-icon" ${app?`data-open="${node.app}"`:`data-path="${esc(node.path)}"`} aria-label="Open ${esc(label)}">${wrapIcon(app?app.icon:fileIcon(node))}<span class="desktop-label">${esc(label)}</span>${!app&&node.kind==='folder'?`<small>${node.children.length} items</small>`:''}</button>`;
    }).join('');
  }
