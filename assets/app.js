/* 90N Desktop — original, dependency-free interface. Content is generated from Desktop/. */
(() => {
  'use strict';
  ...
  const totalImages = [...nodes.values()].filter(n => n.kind === 'image').length;
  ...
  const appColors = {about:['#ffcc84','#e9946c'],experience:['#c2936b','#7d523c'],skills:['#404855','#131a28'],chat:['#b990f7','#7651de'],contact:['#70c8ff','#267cef'],settings:['#aeb7c4','#667185'],github:['#4d5363','#181c25'],help:['#74b8ff','#427af2'],file:['#ffffff','#dce5f4'],image:['#b5e6c3','#349d84'],music:['#ff879f','#da4b77'],video:['#9379e2','#5240a1']};
  let graphicId = 0;
  function appIcon(name) {
    ...
    const glyph = {about:'user',experience:'briefcase',skills:'terminal',chat:'sparkle',contact:'mail',settings:'sliders',github:'github',help:'help',file:'file',image:'image',music:'music',video:'play'}[name] || 'file';
  ...
  const apps = {
    about: {name:'About Noyon',icon:'about',title:'Welcome to my desktop',render:renderWelcome},
    profile: {name:'Profile',icon:'about',title:'About — Md Nazim Uddin Noyon',render:renderProfile},
    ...
    chat: {name:'Ask 90N',icon:'chat',title:'Ask 90N — Portfolio Assistant',render:renderChat},
  ...
  async function getAI(){if(!aiModulePromise)aiModulePromise=import(new URL('ai-runtime.mjs',assetBase).href).then(runtime=>{aiRuntime=runtime;runtime.subscribe(state=>{aiState=state;updateAIStatus();});return runtime;}).catch(error=>{aiModulePromise=null;throw error;});return aiModulePromise;}
  function aiPanel(){
  ...
  function renderDesktop() {
    $('#desktop-apps').innerHTML = data.desktop.children.map(node => `<button class="desktop-icon" data-path="${esc(node.path)}" aria-label="Open ${esc(node.name)}">${wrapIcon(fileIcon(node))}<span class="desktop-label">${esc(node.name)}</span>${node.kind === 'folder' ? `<small>${node.children.length} items</small>` : ''}</button>`).join('') + ['profile','experience','chat'].map(id => `<button class="desktop-icon" data-open="${id}" aria-label="Open ${esc(apps[id].name)}">${wrapIcon(apps[id].icon)}<span class="desktop-label">${esc(id === 'profile' ? 'About me' : apps[id].name)}</span></button>`).join('');
  }
  function renderDock() {
  ...
  function renderWelcome() {
    return `<div class="welcome-content"><div class="welcome-profile"><img class="profile-photo" src="${esc(pathURL(profile.photo))}" alt="${esc(profile.name)} at an event" width="225" height="225"><h2>${esc(profile.alias)}</h2><p>${esc(profile.role)}<br>Esports · Live production</p><div class="profile-location">${icon('broadcast')}Behind the scenes. On the air.</div></div><div class="welcome-copy"><p class="eyebrow">MD NAZIM UDDIN NOYON · HELLO, THAT’S ME</p><h1>The technical side<br>of <span>going live.</span></h1><p class="intro">${esc(profile.intro)}</p><div class="welcome-actions"><button class="btn primary" data-open="finder">${icon('folder')}Explore projects</button><button class="btn" data-open="contact">Let’s talk ${icon('arrow')}</button></div><div class="welcome-stats">${profile.stats.map(s=>`<div><strong>${esc(s.value)}</strong><span>${esc(s.label)}</span></div>`).join('')}</div><button class="welcome-more" data-open="profile">A little more about me ${icon('right')}</button></div></div><footer class="window-footer welcome-footer"><span class="welcome-status">Open to production collaborations</span><span>Made for a closer look. ${icon('sparkle')}</span></footer>`;
  }
  function renderProfile() { return `<article class="document-content"><div class="about-intro"><img src="${esc(pathURL(profile.photo))}" alt="${esc(profile.name)}"><div><p class="eyebrow">THE PERSON BEHIND 90N</p><h1>${esc(profile.name)}</h1><p>${esc(profile.role)} · ${esc(profile.alias)}</p></div></div><p>${esc(profile.bio)}</p><div class="capability-cards"><div class="capability-card">${icon('broadcast')}<h3>Broadcast engineering</h3><p>Live setup, routing, switching, overlays, and the workflows behind a smooth broadcast.</p></div><div class="capability-card">${icon('play')}<h3>Esports production</h3><p>Free Fire event operations, observer integration, and coordination when it’s time to go live.</p></div><div class="capability-card">${icon('code')}<h3>Automation & code</h3><p>Custom tools, data workflows, and practical software that make production easier to run.</p></div></div><p class="section-label">FIND ME ELSEWHERE</p>${socialLinks()}</article>`; }
  function renderExperience() { return `<article class="document-content"><div class="document-title">${wrapIcon('experience')}<div><h1>Behind the broadcasts.</h1><p>My journey in esports production.</p></div></div><div class="timeline">${profile.experience.map(job=>`<div class="timeline-item"><div class="timeline-date">${esc(job.dates)}</div><div><div class="timeline-company">${esc(job.company)}</div><h2>${esc(job.role)}</h2><p>${esc(job.description)}</p></div></div>`).join('')}</div><div class="experience-note">${icon('broadcast')}Backend operations. Live execution. A show that keeps moving.</div></article>`; }
  function renderSkills() { return `<article class="document-content"><div class="document-title">${wrapIcon('skills')}<div><h1>My production toolkit.</h1><p>Broadcast tools meet a developer’s mindset.</p></div></div>${profile.skills.map(group=>`<section class="skill-section"><h2>${icon(group.icon)}${esc(group.name)}</h2><div><p>${esc(group.description)}</p><div class="chips">${group.items.map(item=>`<span class="chip">${esc(item)}</span>`).join('')}</div></div></section>`).join('')}</article>`; }
  function renderContact() { return `<article class="contact-content"><div class="contact-heading">${wrapIcon('contact')}<div><h1>Let’s make it happen.</h1><p>A good broadcast starts with a conversation.</p></div></div><p class="contact-lead">Planning an esports event or need help with your production workflow? Tell me what you’re working on.</p><div class="email-card"><div><small>EMAIL ME</small><a href="${esc(safeLink(mail))}">${esc(profile.email)}</a></div><button class="btn" data-action="copy-email" aria-label="Copy email address">${icon('copy')}Copy</button></div><div class="contact-services"><span class="chip">Esports production</span><span class="chip">Broadcast engineering</span><span class="chip">Workflow automation</span></div><a class="btn primary" href="${esc(safeLink(mail))}">${icon('mail')}Start a conversation</a><p class="contact-small">Opens your email app. Include your event dates and production needs.</p><p class="section-label">AROUND THE INTERNET</p>${socialLinks()}</article>`; }
  ...
  function matchAnswer(input){
    ...
    if(/^(who is|tell me about|about|পরিচয়|কে|ke tumi)/u.test(normalized) && /(noyon|nazim|90n|you|him|নয়ন|নয়ন|তুমি)/u.test(normalized))return data.assistant.questions.find(q=>q.id==='about');
    return {answer:data.assistant.fallback,action:'contact',actionLabel:'Contact Noyon'};
  }
