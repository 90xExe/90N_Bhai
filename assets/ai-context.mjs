...
export function buildMessages(data, history) {
  ...
  const generic=new Set(['noyon','nazim','what','does','about','how','his','can','the','that','with']);
  const score=text=>tokens(text).reduce((n,t)=>n+(query.has(t)&&t.length>2&&!generic.has(t)?1:0),0);
  const p=data.profile;
  const facts=[`Name: ${p.name} (${p.alias}). Role: ${p.role}.`, `Noyon describes his work: ${p.bio}`,
